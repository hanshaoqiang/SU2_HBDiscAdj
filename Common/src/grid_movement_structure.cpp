	...
	for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
        ...
        ((config->GetMarker_All_DV(iMarker) == YES) && (Kind_SU2 == SU2_DOT))) {
			for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
				...
				for (iDim = 0; iDim < nDim; iDim++) {
				  //cout << "if cond met inside set boundary disp" << endl;
					total_index = iPoint*nDim + iDim;
...
void CVolumetricMovement::Rigid_Rotation(CGeometry *geometry, CConfig *config,
  ...
  if (rank == MASTER_NODE && iter == 0) {
   ...
   ordering is rotation about the x-axis, y-axis, then z-axis. ---*/
    ...
    if (iPoint== 0 )
      cout << "NewGridVel[0][iZone =" << iZone << "]= " << newGridVel[0] << endl;
...
void CVolumetricMovement::Rigid_Pitching(CGeometry *geometry, CConfig *config, unsigned short iZone, unsigned long iter) {
  ...
  /* dtheta = -Ampl[0]*(sin(Omega[0]*time_new + Phase[0]) - sin(Omega[0]*time_old + Phase[0])) -Ampl2[0]*(sin(Omega2[0]*time_new + Phase[0]) - sin(Omega2[ \
                                                                                                                                                       0]*time_old + Phase[0]));
  dphi   = -Ampl[1]*(sin(Omega[1]*time_new + Phase[1]) - sin(Omega[1]*time_old + Phase[1])) -Ampl2[1]*(sin(Omega2[1]*time_new + Phase[1]) - sin(Omega2[\
                                                                                                                                                       1]*time_old + Phase[1]));
  dpsi   = -Ampl[2]*(sin(Omega[2]*time_new + Phase[2]) - sin(Omega[2]*time_old + Phase[2])) -Ampl2[2]*(sin(Omega2[2]*time_new + Phase[2]) - sin(Omega2[\
  2]*time_old + Phase[2]));*/
