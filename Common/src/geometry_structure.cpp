	...
	if (config->GetUnsteady_Simulation()==SPECTRAL_METHOD)
	  {
	    nExtIter = config->GetnTimeInstances();
	    //nExtIter = 1;
	    cout << "**********nExtIter in SetSensitivity = " << nExtIter << endl;
	    total_T = config->GetSpectralMethod_Period();
	    delta_T = total_T/nExtIter;
	  }
    //    delta_T  = config->GetDelta_UnstTimeND();
    //    delta_T  = 1.0;
    //total_T  = (su2double)nExtIter*delta_T;
...
<<<<<<< HEAD
      ...
      cout << "******Inside SetSesitivity - Reading restart file = " << filename << endl;
...
<<<<<<< HEAD
        if (iPoint_Local >= 0){
          point_line >> index;
	  if(iPoint_Local==0)
	    cout << "Inside SetSensitivity - number of skip columns for Obj  func = " << skipVar << endl;
	    ...
	    if (config->GetUnsteady_Simulation()==SPECTRAL_METHOD)
	      Sensitivity[iPoint_Local*nDim+iDim] += Sens/nZone;
	    else
	      // For unsteady simulations already accumulated in the last iter file
	      if (iExtIter == nExtIter-1)
