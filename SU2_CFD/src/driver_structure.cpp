...
void CDriver::Iteration_Preprocessing(CIteration **iteration_container, CConfig **config, unsigned short iZone) {
  ...
  switch (config[iZone]->GetKind_Solver()) {
    ...
    case DISC_ADJ_EULER: case DISC_ADJ_NAVIER_STOKES: case DISC_ADJ_RANS:
      ...
      if (config[iZone]->GetUnsteady_Simulation()==SPECTRAL_METHOD)
	{
	  cout << "Spectral Method so enforcing multi Zone Disc Adjoint " << endl;
	  iteration_container[iZone] = new CDiscAdjMultiZoneMeanFlowIteration(config[iZone]);
	}
      else
...
/*---- Spectral Discrete Adjoint Driver making use of Discrete muti zone Adjoint ----*/
CSpectralDiscAdjDriver::CSpectralDiscAdjDriver(CIteration **iteration_container,
                                 CSolver ****solver_container,
                                 CGeometry ***geometry_container,
                                 CIntegration ***integration_container,
                                 CNumerics *****numerics_container,
                                 ...
                                 unsigned short val_nZone) : CDriver(iteration_container,
                                                                     ...
                                                                     val_nZone)
{
  cout << "In Spectral discrete adjoint solver *******------" << endl;
    meanflow_iteration = new CIteration*[nZone];
    turbulent = config_container[ZONE_0]->GetKind_Solver() == DISC_ADJ_RANS;
    
    unsigned short iZone;
    switch (config_container[ZONE_0]->GetKind_Solver()) {
        case DISC_ADJ_RANS: case DISC_ADJ_EULER: case DISC_ADJ_NAVIER_STOKES:
            for (iZone = 0; iZone < nZone; iZone++){
                meanflow_iteration[iZone] = new CMeanFlowIteration(config_container[iZone]);
            }
            break;
        default:
            break;
...
CSpectralDiscAdjDriver::~CSpectralDiscAdjDriver(void) { }

void CSpectralDiscAdjDriver::Run(CIteration **iteration_container,
                          COutput *output,
                          CIntegration ***integration_container,
                          CGeometry ***geometry_container,
                          CSolver ****solver_container,
    ...
    /*--- Preprocess Begin ---*/
    for (iZone = 0; iZone < nZone; iZone++)
        iteration_container[iZone]->Preprocess_1(output, integration_container, geometry_container,
                                               solver_container, numerics_container, config_container,
                                               surface_movement, grid_movement, FFDBox, iZone);
    
    /*--- Set Recording Discrete Adjoint loop for MultiZone ---*/
    SetRecording(iteration_container, output, integration_container, geometry_container,
                 solver_container, numerics_container, config_container,
                 surface_movement, grid_movement, FFDBox,iteration_container[ZONE_0]->FLOW_VARIABLES);
    
    for (iZone = 0; iZone < nZone; iZone++)
        iteration_container[iZone]->Preprocess_output(output, integration_container, geometry_container,
                                                 solver_container, numerics_container, config_container,
                                                 surface_movement, grid_movement, FFDBox, iZone);
    /*--- Preprocess Ends ---*/
    ...
    for (iZone = 0; iZone < nZone; iZone++)
        iteration_container[iZone]->InitializeAdjoint(solver_container, geometry_container,
                                             config_container, iZone);
    ...
    for (iZone = 0; iZone < nZone; iZone++)
        iteration_container[iZone]->ExtractAdjoint(output, integration_container, geometry_container,
                                                      solver_container, numerics_container, config_container,
                                                      surface_movement, grid_movement, FFDBox, iZone);
    ...
    /*---- Set the convergence criteria (only residual possible) ---*/
    if ((ExtIter+1 >= config_container[ZONE_0]->GetnExtIter()) || (integration_container[ZONE_0][ADJFLOW_SOL]->GetConvergence()) ||
        ((ExtIter % config_container[ZONE_0]->GetWrt_Sol_Freq() == 0))) {
    
        /*--- Zone 1  passed the convergence criteria ---*/
        
        /*--- Record one mean flow iteration with geometry variables as input ---*/
        
        SetRecording(iteration_container, output, integration_container, geometry_container,
                     solver_container, numerics_container, config_container,
                     surface_movement, grid_movement, FFDBox,iteration_container[ZONE_0]->GEOMETRY_VARIABLES);
...
void CSpectralDiscAdjDriver::SetRecording(CIteration **iteration_container,
                                          COutput *output,
                                          CIntegration ***integration_container,
                                          CGeometry ***geometry_container,
                                          CSolver ****solver_container,
    ...
    for (iZone = 0; iZone < nZone; iZone++)     {
        iteration_container[iZone]->PrepareForRecording(output, integration_container, geometry_container,
                                                        solver_container, numerics_container, config_container,
                                                        surface_movement, grid_movement, FFDBox, iZone, kind_recording);
        
        /*--- Run one iteration while tape is passive - this clears all indices ---*/
        iteration_container[iZone]->SetDependencies(solver_container, geometry_container, config_container, iZone, ALL_VARIABLES);
        
        if(config_container[ZONE_0]->GetUnsteady_Simulation() == SPECTRAL_METHOD)
            SetSpectralMethod(geometry_container, solver_container, config_container, nZone, iZone);
        
        meanflow_iteration[iZone]->Iterate(output,integration_container,geometry_container,solver_container,numerics_container, config_container,surface_movement,grid_movement,FFDBox,iZone);
    ...
    for (iZone = 0; iZone < nZone; iZone++)
        iteration_container[iZone]->SetRecordingStatus(iZone,kind_recording);
}
