...
<<<<<<< HEAD
  if (val_nZone == SINGLE_ZONE && config_container[ZONE_0]->GetUnsteady_Simulation() != SPECTRAL_METHOD) {
=======
...
<<<<<<< HEAD
      if (config_container[ZONE_0]->GetKind_Solver()==EULER  || config_container[ZONE_0]->GetKind_Solver()==NAVIER_STOKES || config_container[ZONE_0]->GetKind_Solver()==RANS) {
          ...
          *driver = new CSpectralDriver(iteration_container, solver_container, geometry_container,
          ...
          if (rank == MASTER_NODE) cout << "Instantiating a Discrete Adjoint spectral method driver for the problem. " << endl;
          *driver = new CSpectralDiscAdjDriver(iteration_container, solver_container, geometry_container,
                                        integration_container, numerics_container, config_container, val_nZone);
      }
